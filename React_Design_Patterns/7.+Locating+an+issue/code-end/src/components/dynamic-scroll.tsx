...
// styled containers can also take in props and conditionally change the styled properties
...
const DynamicScroll = ({ content }: { content: ReactNode }) => {
  ...
  // event handler that calculates the newPosition and sets the larger of the number to state variable
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    const newPosition = calculatePosition(e.currentTarget.scrollTop);
    setPosition(Math.max(113, newPosition));
  };

  // blockColor = function to return rgb color
